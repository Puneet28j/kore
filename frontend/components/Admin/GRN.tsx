...
// service for GRN-related API calls
import { grnService } from "../../services/grnService";

type RefType = "PO" | "CAT";

type RefItem = {
  id: string; // "PO-1023" or "CAT-2045"
...
// Refs will be fetched from backend (POs + catalog items)
// we still use the same RefItem structure for dropdown

// state is declared later inside component
...
const GRN: React.FC = () => {
  ...
  // reference to hidden input used when scanning via hardware
  const hiddenInputRef = useRef<HTMLInputElement>(null);

  // focus scanner input whenever reference selection changes
  useEffect(() => {
    if (selectedRef) {
  ...
  // GRN History fetched from backend
  ...
  // when the search term changes we re-fetch the reference list from backend
  useEffect(() => {
    let cancelled = false;
    const fetchRefs = async () => {
      setRefsLoading(true);
      setRefs([]); // clear old options while fetching
      try {
        const res = await grnService.listReferences(searchTerm);
        if (cancelled) return;
        // backend returns an array of objects compatible with RefItem
        const list: RefItem[] = (res.data || [])
          ...
          .filter((x) => x.refType === "PO"); // only show purchase orders
        setRefs(list);
      } catch (err) {
        console.error(err);
  ...
  // we simply use refs for the dropdown - no further filtering needed
  const filteredRefs = refs; // kept for naming consistency

  const currentCount = currentPairs.length;

  const canSubmit =
    !!selectedRef && cartons.length > 0 && currentPairs.length === 0;
  ...
  const validatePair = (codeRaw: string) => {
    ...
    if (currentPairs.includes(code))
      return "Duplicate in current carton not allowed.";
  ...
  const submitGRN = () => {
    ...
    const submitPromise = async () => {
      ...
      setGrnHistory((prev) => [
        {
          grnNo: `GRN-${todayYYYYMMDD()}-${Math.floor(
            Math.random() * 900 + 100
          )}`,
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
            ...
            <div className="flex flex-wrap gap-2 w-full lg:w-auto">
              <div className="relative flex-1 lg:w-72">
                ...
                <input
                  type="text"
                  placeholder="Search PO/Catalog (backend)"
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div className="flex items-center gap-2">
                ...
                <p className="font-bold text-slate-900">
                  Step 1: Select PO / Catalogue Reference
                </p>
              </div>
              <div className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                SELECTED:{" "}
                <span className="text-slate-800">
                  {selectedRef ? selectedRef.id : "None"}
                </span>
              </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div className="lg:col-span-2">
                <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">
                  Select Purchase Order (only PO are shown)
                </label>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div className="flex items-center gap-2">
                ...
                <p className="font-bold text-slate-900">
                  Step 2: Pair Barcode Scanning (scanner only)
                </p>
              </div>
            ...
            <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="lg:col-span-1">
                ...
                {/* hidden field receives scanner input; focus via button */}
                <input
                  ref={hiddenInputRef}
                  type="text"
                  autoComplete="off"
                ...
                <p className="text-xs text-slate-500 mt-1 italic">
                  Click &ldquo;Scan&rdquo; then use your barcode scanner –
                  typing not required.
                </p>
                ...
                {currentCount > 0 && currentCount < 24 && (
                  <div className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-sm">
                    <p className="font-bold text-amber-800">
                      ⚠ Partial carton not allowed
                    </p>
                    <p className="text-amber-700">
                      Current carton must reach <b>24/24</b>, otherwise GRN
                      Submit will remain disabled.
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-5 flex items-center justify-between flex-wrap gap-2">
              <div className="flex items-center gap-2">
                ...
                <p className="font-bold text-slate-900">
                  Step 3: Completed Cartons (24/24 locked)
                </p>
              </div>
              <span className="text-xs font-black text-slate-400 uppercase tracking-widest">
                Total Cartons:{" "}
                <span className="text-slate-800">{cartons.length}</span>
              </span>
            </div>
                          ...
                          <td className="px-6 py-4 text-right">
                            <div className="inline-flex items-center gap-2">
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  removeCompletedCarton(c.cartonBarcode)
                                }
            ...
            <div className="flex flex-col lg:flex-row gap-3 items-start lg:items-center justify-between">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Submit only when all cartons are complete and current carton
                    is empty.
                    <br />
                    <b>Stock entry will be created carton-wise</b> (not
                    pair-wise).
                  </p>
                </div>
              </div>
